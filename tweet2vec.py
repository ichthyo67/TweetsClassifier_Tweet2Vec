...
def init_params(n_chars):
    ...
    # lookup table
    params['Wc'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(n_chars, CHAR_DIM)).astype('float32'), name='Wc', borrow=True) #added borrow=T

    # f-GRU
    params['W_c2w_f_r'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(CHAR_DIM, HDIM)).astype('float32'), name='W_c2w_f_r', borrow=True)
    params['W_c2w_f_z'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(CHAR_DIM, HDIM)).astype('float32'), name='W_c2w_f_z', borrow=True)
    params['W_c2w_f_h'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(CHAR_DIM, HDIM)).astype('float32'), name='W_c2w_f_h', borrow=True) #added borrow=T
    ...
    params['U_c2w_f_r'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(HDIM, HDIM)).astype('float32'), name='U_c2w_f_r', borrow=True)
    params['U_c2w_f_z'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(HDIM, HDIM)).astype('float32'), name='U_c2w_f_z', borrow=True)
    params['U_c2w_f_h'] = theano.shared(np.random.normal(loc=0., scale=SCALE, size=(HDIM, HDIM)).astype('float32'), name='U_c2w_f_h', borrow=True) #added borrow=T
